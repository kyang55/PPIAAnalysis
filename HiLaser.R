...
spectra <- transformIntensity(CalDigestHiLaser,method="sqrt")
...
peaks <- binPeaks(peaks, tolerance=tol)
...
rm(featureMatrix, peaks, spectra, baseline, avgSpectra, noise, samples)


write.csv(featureMatrix, "test.csv")
